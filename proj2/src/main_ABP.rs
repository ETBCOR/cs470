...
impl GameBoard {
    ...
    fn drop_piece_new_board(&self, spot: Spot, col: isize) -> Option<GameBoard> {
    ...
    fn best_move(&self, spot: Spot) -> Option<isize> {
        match self.alpha_beta(MAX_DEPTH, isize::MIN, isize::MAX, spot) {
            Some(m) if m.col >= 0 => Some(m.col),
            _ => None,
        }
    }
    ...
    fn alpha_beta(
        ...
        // println!(
        //     "[alpha_beta]: (depth: {depth}, a: {alpha}, b: {beta}, spot: {:?}){}",
        //     spot,
        //     self.as_text()
        // );
        if depth == 0 || !self.open_spot() || self.score() == Score::O || self.score() == Score::X {
            // println!("depth {depth} terminal case score: {:?}", self.score());
            return Some(Move {
                col: -1,
                score: match self.score() {
                    Score::InProgress(s) => s,
                    Score::O => isize::MIN,
                    Score::X => isize::MAX,
                },
            });
        ...
        match spot {
            Spot::Empty => panic!("Spot::Empty encountered in alpha_beta"),
            Spot::O => {
                let mut mov = None;
                for col in 0..self.grid.dim.col {
                    if let Some(new_board) = self.drop_piece_new_board(spot, col) {
                        // println!("new board was possible on col {col}");
                        if let Some(m) = new_board.alpha_beta(depth - 1, alpha, beta, Spot::X) {
                            if mov == None {
                                // println!("depth {depth} initialized to {}", m.score);
                                mov = Some(Move {
                                    col,
                                    score: m.score,
                                })
                            } else if m.score < mov.unwrap().score {
                                // println!("depth {depth} minimized to {}", m.score);
                                mov = Some(Move {
                                    col,
                                    score: m.score,
                                });
                            ...
                            if mov.unwrap().score < alpha {
                                // println!("depth {depth} hit alpha cutoff");
                                break;
                            }
                            beta = min(beta, mov.unwrap().score);
                        } else {
                            // println!("depth {depth} couldn't recurese!");
                        }
                    }
                }
                // println!("depth {depth} returned {:?}", mov);
                return mov;
            }
            Spot::X => {
                let mut mov = None;
                for col in 0..self.grid.dim.col {
                    if let Some(new_board) = self.drop_piece_new_board(spot, col) {
                        // println!("new board was possible on col {col}");
                        if let Some(m) = new_board.alpha_beta(depth - 1, alpha, beta, Spot::O) {
                            if mov == None {
                                // println!("depth {depth} initialized to {}", m.score);
                                mov = Some(Move {
                                    col,
                                ...
                                // println!("depth {depth} maximized to {}", m.score);
                                mov = Some(Move {
                                    col,
                                    score: m.score,
                                });
                            ...
                            if mov.unwrap().score > beta {
                                // println!("depth {depth} hit beta cutoff");
                                break;
                            }
                            alpha = max(alpha, mov.unwrap().score);
                        } else {
                            // println!("depth {depth} couldn't recurese!");
                        }
                    }
